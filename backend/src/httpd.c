// FROM https://github.com/foxweb/pico
...
void initialize_httpserver(const char *PORT) {
  struct sockaddr_in clientaddr;
  socklen_t addrlen;

  int slot = 0;

  LOG_INFO("Server started %s", ansi_green("http://127.0.0.1:%s\n", PORT,));
...
void start_server(const char *port) {
  struct addrinfo hints = malloc(sizeof(struct addrinfo)), *res, *p;
...
void respond(int slot) {
  ...
  if (rcvd < 0) // receive error
    ...
    LOG_ERROR("Client disconnected upexpectedly.\n");
  else // message received
  {
    ...
    LOG_INFO("[%s] Request recieved on \"%s\"", ansi_cyan(method), ansi_cyan(uri));
